...
- Current delivery stage: Make bounded admission cross the real model-to-tool boundary while keeping bounded reversible work on a direct-delivery fast lane.
- Stage completion boundary: Bounded reversible work reaches delivery without durable control setup; complex work retains synchronous Pre/Post admission, method-family controls, generic replays, transactional installation, and a live trusted denial canary.
- User-visible proof: Bounded reversible work begins without child state or orchestration; missing or invalid durable admission creates nothing; covered durable calls retain exact Pre/Post enforcement.
- Active acceptance slice: REQ-V6-FAST-PACKAGE — publish the exact validated candidate as a non-stable GitHub prerelease.
...
- Priorities: Deliver bounded reversible outcomes directly; prevent voluntary admission bypass, unmetered preparation, control-plane growth, one-attempt unlimited work, method-family churn, and false completion.
...
- Explicit corrections: Audit the complete relevant project-task histories, fix the general cause of simple-task overcomplication, test it, and publish it to the comprono/dont-x-smart-be-smart-skill GitHub repository.
...
- Route one bounded reversible deliverable through direct delivery: no child state, workers, or custom framework; use the declared small behavioral bounds and promote only for an intrinsic durable condition.
...
- Editing a motivating project or encoding its identity in the generic package.

## Operational Envelope

- Applies to: One bounded standalone-skill direct-admission repair and its explicitly authorized v6.0.0-rc.2 GitHub prerelease; no motivating-project edits, recurring service, or stable v6.0.0 release before the live canary.
...
- Resource budget, reserve, and retention: One implementation path, two independent read-only audits, bounded tests, an isolated double-install, and one prerelease; preserve the live-denial canary for stable v6.
...
- Verification commands: python -m unittest discover -s tests -v; project_outcome.py validate, resume, completion; quick_validate.py
- Evidence roots: tests, forward-test responses, isolated installer output, installed-source hashes
...
- A disproven architecture expanded instead of triggering a method comparison | Class: method-family fixation | Evidence: generic chronology showed substantial work continuing after the earliest invalidating transition | Invariant: architecture invalidation stops that family before further material work | Do not repeat: replacing an invalid design with a larger member of the same unproven family
- A bounded reversible deliverable acquired a separate ledger, workers, waits, and a custom publisher after its content checks already passed | Class: control-plane substitution | Evidence: generic incident replay | Invariant: direct delivery remains the default until a concrete promotion condition is observed | Do not repeat: treating an ambiguous write result as permission to build deployment architecture
